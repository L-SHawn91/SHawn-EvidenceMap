...
def make_frames(summary: dict[str, int], entities: list[dict], output_dir: Path) -> list[Path]:
    ...
    image, draw = base_frame("06 / 06", "Try the free CLI—or join early validation")
    ...
    draw_wrapped(draw, "Free validation cohort", (670, 210, 1160, 270), size=31, fill=PURPLE, bold=True)
    draw_wrapped(draw, "Run one bounded public workflow, inspect the outputs, and share onboarding feedback. No private PDFs, patient data, or hosted upload.", (670, 270, 1160, 495), size=25, fill=MUTED)
