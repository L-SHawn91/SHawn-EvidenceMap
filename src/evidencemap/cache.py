...
def get_cached_map(
    ...
    item = payload.get(cache_key(query, limit, ranking_mode))
    if not item:
        return None
...
def set_cached_map(
    ...
    payload = load_cache(cache_path)
    payload[cache_key(evidence_map.query, limit, ranking_mode)] = {
        "ts": time.time(),
...
def cache_key(query: str, limit: int, ranking_mode: str) -> str:
    raw = f"{CACHE_VERSION}|{query.strip().lower()}|{limit}|{ranking_mode}"
...
def map_from_dict(data: dict[str, Any]) -> EvidenceMap:
    ...
    return EvidenceMap(query=data.get("query", ""), papers=papers, rows=rows)
